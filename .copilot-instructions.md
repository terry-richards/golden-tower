# Golden Tower — Hydroponic Grow Tower Design System

## Copilot Instructions for Claude Agent Swarm

> These instructions define the mission, constraints, roles, and iteration
> protocol for a human-in-the-loop agent swarm that designs, validates,
...
The swarm operates with **human-in-the-loop checkpoints** — after each
iteration, results and renders are presented for human GO/NO-GO approval
before continuing (§6.1.2). All design work is done by the agents; the
human reviews visual output and provides direction.
...
| Tool | Purpose |
|---|---|
| **Blender 5.0** (bpy) | 3D modeling, boolean operations, mesh editing, and headless rendering |
| **BMesh** (bmesh) | Low-level mesh construction, manifold checks, volume calculation |
| **Blender Python API** | Parametric geometry generation via scripted modifiers and mesh ops |
| **EEVEE** | Fast headless rendering for visual validation (multi-view PNGs) |
| **Cycles** (optional) | High-quality renders for final deliverables |
| **bpy.ops.mesh.bisect** | Cross-section slicing for internal geometry validation |
| **NumPy** | Golden angle math, geometric calculations |
| **trimesh** | Supplemental STL mesh analysis (watertight, cross-section areas) |
...
### 3.1 Blender Execution Model

All geometry scripts run Blender in **headless mode**:
```bash
blender --background --python script.py
```

Scripts use `bpy` (Blender Python API) for:
- Mesh creation and boolean operations
- Modifier stack (solidify, bevel, mirror, boolean)
- STL/OBJ export
- Headless rendering to PNG (EEVEE for speed, Cycles for quality)

The venv at `./venv/` is used for standalone Python (pytest, trimesh).
Blender uses its own bundled Python — geometry scripts import `bpy` directly.

**Key difference from build123d**: Blender's boolean operations are more
visual and debuggable. When a boolean fails, you can open the .blend file
in the GUI to inspect what went wrong. The agent should save .blend files
alongside STLs for debugging.

All scripts live in `/home/tmr/src/learn/golden-tower/`.
Virtual environment: `./venv/` for pytest/trimesh (standalone Python).
Blender: `/snap/bin/blender` (system install, version 5.0.1).
...
- Writes the master `build_tower.py` and component scripts for Blender (`bpy`).
- Saves `.blend` files to `exports/blend/` for GUI debugging.
- Outputs: STL files, .blend files, parameter configs.
...
├── build_tower.py               ← Main Blender build script (run with blender --background --python)
├── validate_visual.py           ← Blender headless render + cross-section + analysis pipeline
├── components/
...
│   ├── blend/                   ← Blender .blend files for GUI inspection
│   └── renders/                 ← Rendered PNGs (multi-view, cross-sections)
├── reports/
...
│  2. Run visual validation: python validate_visual.py     │
│  3. Engineering validates structure & water flow          │
│  4. Print agent validates manufacturability               │
│  5. Aesthetics agent reviews RENDER IMAGES (not code)     │
│  6. Integration agent runs full test suite                │
│  7. Write iteration summary + score                      │
│  8. Commit, push, then STOP for human review             │
│  9. Human says GO → continue | NO-GO → address feedback  │
│ 10. Decision: ITERATE (go to 1) or PROMOTE               │
└─────────────────────────────────────────────────────────────┘
```

### 6.1.1 Visual Validation (MANDATORY)

After every geometry change, the orchestrator MUST run:
```bash
blender --background --python validate_visual.py
```

This uses Blender's EEVEE renderer to generate in `exports/renders/`:
- **`{name}_front.png`** — Front view render (XZ plane)
- **`{name}_right.png`** — Right side view render (YZ plane)
- **`{name}_top.png`** — Top-down view render (XY plane)
- **`{name}_perspective.png`** — 3/4 isometric view with materials
- **`{name}_cross_{z}mm.png`** — Cross-section renders at key Z-heights
  using `bpy.ops.mesh.bisect` to slice the mesh in-place
- **`{name}_analysis.txt`** — BMesh-based dimensional analysis (volume,
  manifold status, bounding box, wall thickness sampling)

Because these are **real 3D renders** (not matplotlib wireframes), agents
can evaluate actual surface quality, pocket depth, material transitions,
and visual proportions. The orchestrator and all reviewing agents MUST
examine these images before scoring.

**Do not score aesthetics, printability, or water flow without looking at
the renders.** If you cannot see the renders, say so — do not invent scores.

### 6.1.2 Human-in-the-Loop Gate (MANDATORY)

After committing and pushing each iteration, the orchestrator MUST:
1. Present a summary of what changed, the current scores, and any open issues.
2. List the render files generated in `exports/renders/`.
3. **STOP and ask the human**: "Iteration N complete. Renders are in
   exports/renders/. Please review and reply GO to continue or provide
   feedback."
4. **Wait for human response** before starting the next iteration.

This is not optional. The swarm does NOT continue autonomously between
iterations. Each iteration requires explicit human approval.
...
4. **All geometry changes** must be made in the parametric model
   (`tower_params.py` + component scripts run via Blender), never by
   manual STL editing. Always save .blend files alongside STLs.
...
- [ ] All .blend files in `exports/blend/` (for GUI inspection and further work)
...
## 9. Key Design Patterns for Blender (bpy)

```python
# Golden angle node placement pattern — used by all component scripts
import math
import sys
import os

# tower_params.py must be importable
...
### 9.1 Blender Component Script Pattern

Every component script follows this structure:
```python
#!/usr/bin/env python3
"""
Component Name — Golden Tower
Run: blender --background --python components/component_name.py
"""
import bpy
import bmesh
import math
...
# Import tower_params (standalone Python — not bpy-dependent)
...
def build_component():
    """Build the component geometry using bpy/bmesh."""
    clear_scene()
    
    # ... geometry construction using bmesh or bpy.ops.mesh ...
    
    # Apply modifiers, clean up
    bpy.ops.object.select_all(action='SELECT')
    return bpy.context.selected_objects[0]

def export(obj, name):
    """Export to STL and save .blend file."""
...
if __name__ == '__main__':
    obj = build_component()
    export(obj, 'component_name')
```

### 9.2 Boolean Operations in Blender

```python
# Blender boolean pattern — more debuggable than build123d
def boolean_op(target_obj, tool_obj, operation='DIFFERENCE'):
...
### 9.3 Cross-Section Visualization

```python
# Slice a mesh at height Z using bpy.ops.mesh.bisect
def cross_section_at_z(obj, z_height):
    """Duplicate object and bisect at z_height for cross-section view."""
    # Duplicate
    bpy.ops.object.select_all(action='DESELECT')
    obj.select_set(True)
    ...
    bpy.ops.mesh.bisect(
        plane_co=(0, 0, z_height),
        plane_no=(0, 0, 1),
        clear_inner=True,   # remove below
        clear_outer=False,   # keep above
    )
    bpy.ops.object.mode_set(mode='OBJECT')
    return section
```

### 9.4 Headless Rendering Pattern

```python
def setup_render_scene(obj, camera_location, camera_target=(0, 0, 100)):
    """Set up camera, lights, and render settings for headless EEVEE render."""
    ...
    # Material on object (if none)
    if not obj.data.materials:
        mat = bpy.data.materials.new('TowerMaterial')
        mat.use_nodes = True
        bsdf = mat.node_tree.nodes['Principled BSDF']
        bsdf.inputs['Base Color'].default_value = (0.29, 0.56, 0.85, 1.0)
...
- If a Blender boolean fails → switch solver from 'EXACT' to 'FAST' or
  vice versa; add slight offsets to avoid co-planar faces; check normals
  are consistent (`bpy.ops.mesh.normals_make_consistent()`); save the
  .blend file for manual debugging and log the error.
- If a mesh fails manifold check → use BMesh to find non-manifold edges
  (`[e for e in bm.edges if not e.is_manifold]`); fill holes with
  `bpy.ops.mesh.fill()`; re-export and re-test.
...
- If interlock is too tight/loose → adjust INTERLOCK_CLEARANCE in 0.05 mm
  increments.
- If aesthetic score is low → add bevel modifiers (min width 2 mm), smooth
  transitions, and subdivision surface for organic curves at pocket openings.
- If Blender crashes in headless mode → check `blender --debug --background
  --python script.py` for detailed error output; reduce mesh complexity or
  boolean count; break complex operations into sequential steps.
...
| **build123d** | ~~Former CAD kernel — replaced by Blender in iteration 4~~ |
| **Blender (bpy)** | Python-scriptable 3D modeling, rendering, and mesh analysis application |
| **BMesh** | Blender's low-level mesh editing API for programmatic geometry construction |
| **EEVEE** | Blender's real-time render engine — used for fast headless validation renders |
