...
| Vertical pitch per node | Derived from desired segment height | Maintain uniform vertical spacing so leaves don't shadow each other |
...
are stacked. Target segment count: **6–10 segments** for a full tower.

### 2.2 Component Inventory

| Component | Count per tower | Description |
|---|---|---|
| **Standard Segment** | 6–10 | 3 planting pockets spiraling at golden angle; male/female interlock top/bottom; central tube pass-through |
| **Top Cap** | 1 | Deflects water from the central irrigation tube outward and down into the topmost segment's pockets; aesthetic finial |
...
| **Central Tube** | 1 | Smooth-bore tube running the full tower height; drip holes or slots aligned with each pocket; may be printed or off-the-shelf PVC/silicone |
...
- Pocket angle: tilted **15–25°** outward from vertical for drainage and
  light exposure.
- Each pocket receives water via gravity from the central tube or the
  pocket above; a small channel routes nutrient solution into the pocket
  and drains overflow to the pocket below or back to the reservoir.

### 2.5 Water Flow

```
Pump → Quick-disconnect → Bottom segment → Central tube (up)
  → Top cap deflector → Drips/flows down outer spiral pockets
  → Drains back to reservoir through bottom segment
```
...
**Responsibility**: High-level design decisions and parametric model structure.
...
├── components/
│   ├── segment.py               ← Standard segment geometry
...
│   └── central_tube.py          ← Central irrigation tube
├── tests/
...
SEGMENT_OUTER_DIAMETER = 180.0  # mm — max outer envelope
CENTRAL_TUBE_OD = 32.0        # mm — outer diameter of central tube
...
def place_nodes(n_nodes, radius, vertical_pitch):
    """Generate (angle, z) positions for n nodes following golden angle."""
    positions = []
...
# Interlock: use Rotation about Z to enforce golden-angle offset
# segment_upper = segment.rotate(Axis.Z, INTERLOCK_ROTATION)
```
...
- Assert central tube passes through all segments without interference
