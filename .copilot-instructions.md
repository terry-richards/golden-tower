...
| **matplotlib** | Headless rendering for visual validation pipeline |
| **Pillow** | Image generation for render PNGs |
| **shapely** | 2D polygon area calculations for cross-section analysis |
...
├── validate_visual.py           ← Render + cross-section + analysis pipeline
├── components/
...
This generates in `exports/renders/`:
- **`{name}_ortho.png`** — 4-view orthographic renders (front, right, top, perspective)
- **`{name}_cross_sections.png`** — 2D slices at 5 Z-heights with reference overlays
- **`{name}_analysis.txt`** — dimensional spot-checks and fill ratio

The orchestrator and all reviewing agents MUST examine these images before
scoring. **Do not score aesthetics, printability, or water flow without
looking at the renders.** If you cannot see the renders, say so — do not
invent scores.
